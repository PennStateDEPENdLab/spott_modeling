...
#     file ="/Users/maruofan/Documents/GitHub/spott_modeling/data/Testing_VI_sim.RData")

# load("/Users/ruofanma/Documents/GitHub/spott_modeling/data/Testing_VI_sim.RData") #labptop
# load("/Users/maruofan/Documents/GitHub/spott_modeling/data/Testing_VI_sim.RData") #desktop
```
...
```{r}
  ...
  choice_1 <- all_df %>% filter(choice ==1 & reward ==1) %>% group_by(replication) %>% 
    mutate(reward_interval = ifelse(is.na(lag(timestep)), timestep*50, (timestep - lag(timestep))*50)) %>% 
    mutate(programmed_interval = head((task_environment[[1]]$programmed_VI[,1]), tally(cur_data())))
  
  choice_2 <- all_df %>% filter(choice ==2 & reward ==1) %>% group_by(replication) %>% 
    mutate(reward_interval = ifelse(is.na(lag(timestep)), timestep*50, (timestep - lag(timestep))*50)) %>% 
    mutate(programmed_interval = head((task_environment[[1]]$programmed_VI[,2]), tally(cur_data())))
```

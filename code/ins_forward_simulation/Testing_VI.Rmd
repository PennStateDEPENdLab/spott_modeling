...
```{r}
  ...
  choice_1 <- all_df %>% filter(choice ==1 & reward ==1) %>% group_by(replication) %>% 
    mutate(reward_interval = ifelse(is.na(lag(timestep)), timestep, (timestep - lag(timestep))*50)) %>% 
    mutate(programmed_interval = head((task_environment[[1]]$programmed_VI[,1])/0.05, tally(cur_data())))
  
  choice_2 <- all_df %>% filter(choice ==2 & reward ==1) %>% group_by(replication) %>% 
    mutate(reward_interval = ifelse(is.na(lag(timestep)), timestep, (timestep - lag(timestep))*50)) %>% 
    mutate(programmed_interval = head((task_environment[[1]]$programmed_VI[,2])/0.05, tally(cur_data())))
```

...
library(tidyverse)
source("c:/Users/zzo1/Dropbox/GitLAb/spott_modeling/spott_modeling/code/ins_forward_simulation/ins_simulation_functions.R")
source("c:/Users/zzo1/Dropbox/GitLAb/spott_modeling/spott_modeling/code/ins_forward_simulation/ins_learning_choice_rulesUpdated.R")
...
# stan_population <- sim_spott_free_operant_group(nsubjects=20, task_environment = task_environment,
#                                                 parameters=list( #use this list to provide expressions that are evaluated to generate group parameter distributions
#                                                   beta=expression(rep(1, nsubjects)))
# )
...
# set.seed(100)
# #run the model at these parameter settings.
# ins_results <- ins_wins(c(alpha=0.1, gamma=8, nu=0, beta=30, cost=0, kappa=2), fixed=NULL, task_environment, optimize=FALSE)
# 
# #get summary statistics
# sstats <- get_sim_stats(ins_results, task_environment)
# sum_df <- sstats$sum_df

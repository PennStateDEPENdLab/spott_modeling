function [overall, prompt_level]=extract_group_statistics(s_array, overall_fname, prompt_level_fname)
...
if nargin < 3, prompt_level_fname='suuvid_prompt_outputs.csv'; end
...
prompt_stats=cell(1,ns);
for i = 1:ns
  this_subj = s_array{i};
  n_obs = size(this_subj.y,2);

  %use 'asc_prompt' to clarify that prompt from VBA is the fitted prompt number, which may not correspond to csv input numbering if there are skips
  ...
  prompt_stats{i} = t_tbl;  
end

prompt_level = vertcat(prompt_stats{:});
writetable(prompt_level, prompt_level_fname);
